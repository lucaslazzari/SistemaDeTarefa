...
{
    public class GetByStatusAssignmentTest
    {
        [Fact]
        public async Task AssignmentExistByStatus_ExecutedByStatus_ReturnAssignmentViewModel()
        {
            // Arrange
            ...
            var status = EnumAssignmentStatus.Pending;

            var assignmentRepositoryMock = new Mock<IAssignmentRepository>();
            assignmentRepositoryMock.Setup(a => a.GetByStatusAsync(status).Result).Returns(assignments);

            var assignmentService = new AssignmentService(assignmentRepositoryMock.Object);

            // Act
            var assignmentViewModel = await assignmentService.GetByStatusAsync(status);
            ...
            assignmentRepositoryMock.Verify(a => a.GetByStatusAsync(status).Result, Times.Once);
        }
    }
}
